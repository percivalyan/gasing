      ...
      <div class="col-sm-12">
        ...
        <div class="card shadow-sm border-0">
          <div class="card-header">
            <h5 class="mb-0">Buat Penugasan Guru ↔ Siswa</h5>
          </div>
          <div class="card-body">
            <form action="{{ route('assign.store') }}" method="POST" novalidate>
              @csrf
              ...
              <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="d-flex gap-2">
                  ...
                  <input type="text" id="filter-name" class="form-control form-control-sm" placeholder="Cari nama...">
                </div>
              </div>
              ...
              <div class="table-responsive" style="max-height: 420px; overflow:auto;">
                <table class="table table-bordered align-middle mb-0" id="students-table">
                  ...
                  <tbody>
                    ...
                    <tr data-name="{{ Str::lower($s->name) }}" data-level="{{ $s->school_level }}">
                      <td>
                        <input type="checkbox" name="student_course_ids[]"
                               value="{{ $s->id }}"
                               {{ in_array($s->id, old('student_course_ids', [])) ? 'checked' : '' }}>
                      </td>
                      <td>{{ $s->name }}</td>
                      <td><span class="badge bg-light text-dark">{{ $s->school_level ?? '-' }}</span></td>
              ...
              @error('student_course_ids') <small class="text-danger d-block mt-2">{{ $message }}</small> @enderror
...
<script>
  ...
  document.getElementById('check-all')?.addEventListener('change', function(){
    document.querySelectorAll('#students-table tbody input[type="checkbox"]').forEach(cb => cb.checked = this.checked);
  });
